# This file is to be patched when building the docker image.
# encoding: utf-8

"""Block item container, used by body, cell, header, etc.

Block level items are things like paragraph and table, although there
are a few other specialized ones like structured document tags.
"""
...
class BlockItemContainer(Parented):
    ...
    These containers include _Body, _Cell, header, footer, footnote,
    endnote, comment, and text box objects. Provides the shared
    functionality to add a block item like a paragraph or table.
    """
    ...
    def add_paragraph(self, text="", style=None):
        ...
        If *style* is |None|, no paragraph style is applied, which has
        the same effect as applying the 'Normal' style.
        """
    ...
    def add_table(self, rows, cols, width):
        ...
        newly appended to the content in this container.
    ...
    @property
    def elements(self):
        """A list containing the elements in this container (paragraph and
    ...
    def abstractNumIds(self):
        return [
            numId
            for numId in self.part.numbering_part.element.iterchildren(
                qn("w:abstractNum")
            )
        ]
